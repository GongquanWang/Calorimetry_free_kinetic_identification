...
# Training Q_heat
X_heating_train, X_heating_test, Y_cooling_train, Y_cooling_test, T_train, T_test, Y_heating_train, Y_heating_test = train_test_split(
    X_heating, Y_cooling_predict_scaled, T_ratio, Y_heating, test_size=0.2, shuffle=False)
...
scaler_T_ratio = MinMaxScaler(feature_range=(0, 1))
T_train_scaled = scaler_T_ratio.fit_transform(T_train)
T_test_scaled = scaler_T_ratio.transform(T_test)
...
if not os.path.exists('Trained_model/Model_heating.h5'):
    ...
    CNN1 = layers.Conv1D(32, 5, padding='same', activation='relu')(time_input)
    CNN1 = layers.MaxPooling1D(2)(CNN1)
    CNN2 = layers.Conv1D(64, 5, padding='same', activation='relu')(CNN1)
    CNN2 = layers.MaxPooling1D(2)(CNN2)
    LSTM1 = layers.LSTM(64, return_sequences=True)(CNN2)
    LSTM2 = layers.LSTM(64, return_sequences=False)(LSTM1)

    features_input1 = layers.Input(shape=(3,))
    Dense1 = layers.Dense(32, activation='relu')(features_input1)
    features_input2 = layers.Input(shape=(1,))
    Dense2 = layers.Dense(32, activation='relu')(features_input2)
    combined = layers.concatenate([LSTM2, Dense1, Dense2])

    Dense3 = layers.Dense(32, activation='relu')(combined)
    Dense3 = layers.Dense(16, activation='relu')(Dense3)
    output = layers.Dense(1, activation='linear')(Dense3)
    model_heating = models.Model(inputs=[time_input, features_input1, features_input2], outputs=output)
    ...
    model_heating.fit(
        [X_heating_train, Y_cooling_train, T_train_scaled],
        Y_heating_train,
        epochs=500,
        validation_data=([X_heating_test, Y_cooling_test, T_test_scaled], Y_heating_test),
        batch_size=64,
        shuffle=True,
        validation_freq=1)
...
joblib.dump(scaler_T_ratio, 'Scaler_parameter/scaler_T_ratio.pkl')
